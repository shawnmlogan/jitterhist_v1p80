...
if ((pdoubles_array = (double *) calloc(number_of_data_columns,sizeof(double))) == NULL)
	...
	printf("In moving_average, requesting num_moving_average_samples = %d\n",num_moving_average_samples);
	printf("(2 * (long int) num_moving_average_samples + 1) = %ld.\n",(2 * (long int) num_moving_average_samples + 1)); */
		
	if (number_of_input_lines < (2 * (long int) num_moving_average_samples + 1))
		{
		if ((number_of_input_lines % 2) == 0)
			printf("Reduce number of moving average samples from %d to less than %ld.\n",
			num_moving_average_samples,(number_of_input_lines/2 - 1));
		else
			printf("Reduce number of moving average samples from %d to less than %ld.\n",
			num_moving_average_samples,(number_of_input_lines - 1)/2 - 1);
		error_flag = 1;
		}
	else
		{	
		...
		if ((pinput_data_array = malloc(number_of_input_lines * sizeof(double *))) == NULL)
			{
			printf("Error allocating memory for **pinput_data_array in moving_average().\n");
			error_flag = 1;
			}
		else
			{
			...
			if (error_flag == 1)
				printf("Error allocating memory for a member of pointer array pinput_data_array[] in moving_average().\n");
			else
				{
				i = 0;
				j = 0;
