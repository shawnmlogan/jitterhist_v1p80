...
if ((fpw1 = fopen(pfin,"r")) == NULL)
	...
	{
	printf("Input file: %s\n",pfin);
	find_stats_column_one_of_file(pfin,&ave_input_signal,&min_input_signal,
	&max_input_signal,number_of_input_lines);
	printf("Output file: %s\n",pfout);
	}  

/* Read, assign, and check sample frequency */

strcpy(pinput_string,argv[3]);
...
if (atof(pinput_string) >= max_input_signal)
	{
	printf("Threshold value exceeds maximum signal value of %s, re-enter value of %s.\n",add_units(max_input_signal,3,"V",value_string[0]),pinput_string);
	input_error_flag = 1;
	}
else
	{
	if (atof(pinput_string) <= min_input_signal)
		{
		printf("Threshold value is less than minimum signal value of %s, re-enter value of %s.\n",add_units(min_input_signal,3,"V",value_string[0]),pinput_string);
		input_error_flag = 1;
		}
	else
		{
